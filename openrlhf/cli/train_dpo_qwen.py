...
from openrlhf.trainer import QwenDPOTrainer
...
def train(args):
    ...
    processor = get_qwen_processor(args.pretrain, model, "left", strategy, use_fast=not args.disable_fast_tokenizer)
    tokenizer = processor.tokenizer
    strategy.print(model)

    # load weights for ref model
    ref_model = Actor(
        "/fsx-ram/shared/Qwen2-VL-7B-Instruct",
        use_flash_attention_2=args.flash_attn,
        bf16=args.bf16,
        load_in_4bit=args.load_in_4bit,
    ...
    train_dataset = QwenRewardDataset(
        train_data,
        processor,
        args.max_len,
        strategy,
    ...
    eval_dataset = QwenRewardDataset(
        eval_data,
        processor,
        args.max_len,
        strategy,

...
class QwenDPOTrainer(ABC):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    chosen_ids, c_mask, reject_ids, r_mask, c_loss_mask, r_loss_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw, prompt_id_lens = data
                    chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                    c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                    c_loss_mask = c_loss_mask.squeeze(1).to(torch.cuda.current_device())
                    reject_ids = reject_ids.squeeze(1).to(torch.cuda.current_device())
                    r_mask = r_mask.squeeze(1).to(torch.cuda.current_device())
                    r_loss_mask = r_loss_mask.squeeze(1).to(torch.cuda.current_device())
                    chosen_pixel_values = chosen_pixel_values.to(torch.cuda.current_device())
                    rejected_pixel_values = rejected_pixel_values.to(torch.cuda.current_device())
                    chosen_image_thw = chosen_image_thw.to(torch.cuda.current_device())
                    rejected_image_thw = rejected_image_thw.to(torch.cuda.current_device())

                    chosen_logps, rejected_logps, aux_loss, nll_loss = self.concatenated_forward(
                        self.model, chosen_ids, c_mask, reject_ids, r_mask,  c_loss_mask, r_loss_mask ,chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw, prompt_id_lens,
                    )
                    with torch.no_grad():
                        if self.ref_model is not None:
                            reference_chosen_logps, reference_rejected_logps, _, _ = self.concatenated_forward(
                                self.ref_model, chosen_ids, c_mask, reject_ids, r_mask, c_loss_mask, r_loss_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw, prompt_id_lens,
                            )
                        else:
                ...
                # loss = preference_loss * 0 + aux_loss * self.args.aux_loss_coef + 1 * self.args.nll_loss_coef
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask,  c_loss_mask, r_loss_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw, prompt_id_lens):
        """Run the given model on the given batch of inputs, concatenating the chosen and rejected inputs together.

        We do this to avoid doing two forward passes, because it's faster for FSDP.
        """
        input_ids, att_masks, loss_mask, pixel_values, image_thw, prompt_id_lens = self.concatenated_inputs(
            chosen_ids, c_mask, reject_ids, r_mask, c_loss_mask, r_loss_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw, prompt_id_lens
        )
        output = model(input_ids, attention_mask=att_masks, pixel_values=pixel_values, image_grid_thw=image_thw, return_output=True)
        all_logits = output["logits"]
        all_logps_sum, all_logps_mean = self._get_batch_logps(
            all_logits, input_ids, att_masks, loss_mask,
            prompt_id_lens, 
            average_log_prob=False
        )
        if self.mean_log_prob:
            # modified to use the mean
            chosen_logps = all_logps_sum[: chosen_ids.shape[0]]/loss_mask.sum(-1)[:chosen_ids.shape[0]]
            rejected_logps = all_logps_sum[chosen_ids.shape[0] :]/loss_mask.sum(-1)[chosen_ids.shape[0]:]
        else:
    ...
    def concatenated_inputs(self, chosen_ids, c_mask, reject_ids, r_mask,  c_loss_mask, r_loss_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw, prompt_id_lens,):
        ...
        loss_masks = torch.cat((pad_to_length(c_loss_mask, max_length, 0), pad_to_length(r_loss_mask, max_length, 0)), dim=0)
        pixel_values = torch.cat((chosen_pixel_values, rejected_pixel_values), dim=0)
        image_thw = torch.cat((chosen_image_thw, rejected_image_thw), dim=0)
        return inputs_ids, att_masks, loss_masks, pixel_values, image_thw,prompt_id_lens * 2
    ...
    def _get_batch_logps(
        ...
        loss_masks = loss_mask.clone().bool()

...
class QwenRewardModelTrainer(ABC):
    """
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    ...
                    chosen_reward, reject_reward, aux_loss = self.concatenated_forward(
                        self.model, chosen_ids, c_mask, reject_ids, r_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw
                    )
                else:
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw):
        """Run the given model on the given batch of inputs, concatenating the chosen and rejected inputs together.

        We do this to avoid doing two forward passes, because it's faster for FSDP.
        """
        input_ids, att_masks, pixel_values, image_thw = self.concatenated_inputs(chosen_ids, c_mask, reject_ids, r_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw)
        all_values, output = model(input_ids, attention_mask=att_masks, pixel_values=pixel_values, image_thw=image_thw, return_output=True)
        chosen_rewards = all_values[: chosen_ids.shape[0]]
        rejected_rewards = all_values[chosen_ids.shape[0] :]
        aux_loss = output.aux_loss if "aux_loss" in output else []
        return chosen_rewards, rejected_rewards, aux_loss

    def concatenated_inputs(self, chosen_ids, c_mask, reject_ids, r_mask, chosen_pixel_values, rejected_pixel_values, chosen_image_thw, rejected_image_thw):
        ...
        return inputs_ids, att_masks, pixel_values, image_thw
